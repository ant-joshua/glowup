...
import { createVideoFromPrompt } from "./pixverse/runPixverse";
...
function readBody(req: import("node:http").IncomingMessage, limitBytes: number) {
  ...
  for (const field of template.fields) {
    ...
    if (field.type === "list") {
      if (Array.isArray(value)) {
        out[field.key] = value.map((v) => String(v)).filter(Boolean).slice(0, 20);
...
function renderPrompt(template: Template, variables: Record<string, unknown>) {
  ...
  if (Array.isArray(steps)) {
    merged.steps = steps.map((s) => String(s)).filter(Boolean).join(", ");
  }

  return template.promptTemplate.replace(/\{([a-zA-Z0-9_]+)\}/g, (_m, key: string) => {
...
function getSceneDurationSec(template: Template, variables: Record<string, unknown>) {
  const raw = variables.sceneDurationSec;
  const fromVar = typeof raw === "string" && raw.trim() ? Number(raw.trim()) : null;
...
const storePath = process.env.PIXVERSE_WORKER_STORE_PATH?.trim() || "./data/jobs.json";
const port = Number(process.env.PORT ?? "4107");
const maxConcurrency = Math.max(1, Number(process.env.PIXVERSE_WORKER_MAX_CONCURRENCY ?? "1"));
const timeoutSec = Math.max(60, Number(process.env.PIXVERSE_WORKER_TIMEOUT_SEC ?? "900"));
...
async function runJob(jobId: string) {
  ...
  const initialClips: JobClip[] = Array.from({ length: total }).map((_, idx) => ({
  ...
  for (let idx = 0; idx < total; idx += 1) {
    ...
    if (!result.ok) {
      clips[idx] = {
        ...
        finishedAt: nowIso(),
        output: {
          videoId: typeof result.videoId === "number" ? result.videoId : undefined,
        },
        error: {
    ...
    runJob(jobId)
      .catch(async (err) => {
        ...
        const failed: Job = {
          ...
          status: "failed",
          finishedAt: nowIso(),
          error: { message: err instanceof Error ? err.message : "unknown_error" },
        };
        await store.upsert(failed);
      })
      .finally(() => {
